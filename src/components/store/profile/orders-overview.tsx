...
export default function OrdersOverview() {
  return (
    <div className='mt-4 bg-white dark:bg-slate-800 p-4 border shadow-sm'>
      <div className='flex items-center border-b'>
        <div className='inline-block flex-1 py-3 text-xl font-bold'>
          My Orders
        </div>
        <Link href='/profile/orders'>
          <div className='flex items-center text-main-primary dark:text-white text-sm cursor-pointer'>
            View All
        ...
        {menu.map((item) => (
          <Link key={item.link} href={item.link}>
            <div className='relative w-full flex flex-col justify-center items-center cursor-pointer'>
              ...
              />
              <div className='text-main-primary dark:text-white text-sm mt-2'>
                {item.title}
              </div>
            </div>
          </Link>
      ...
      <div className='relative flex items-center py-4 border-t cursor-pointer dark:border-slate-700 border-main-primary'>
        <span className='text-2xl inline-block'>
          <AppealIcon />
        </span>
        <div className='ml-1.5 text-main-primary dark:text-white'>
          My appeal
        </div>
        <span className='absolute right-0 text-main-secondary dark:text-white text-lg'>
          <ArrowIcon />
        </span>
      </div>
